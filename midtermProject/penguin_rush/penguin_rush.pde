...
class Seal extends GameCharacter {
  ...
  void update() {
    ...
    // If the player is moving, the sprite s changed where the rate of change is calculated by using MOD function with frameCount
...
class Penguin extends GameCharacter {
  ...
  void update() {
    ...
    for (int i = 0; i < game.seal.size(); i++) {
      if (distance(game.seal.get(i).x, game.seal.get(i).y) <= 110) {
        // collision
        alive = false;
        background_sound.stop();
        gameOver_sound.play();
    ...
    for (int i = 0; i < game.spike.size(); i++) {
      s = game.spike.get(i);
      if ((x >= s.x && x <= s.x + s.w)||(x+10 >= s.x && x+10 <= s.x + s.w)) {
